...
**云端部署地址**：[https://cswun-datavis-dashboard-f.onrender.com/](https://cswun-datavis-dashboard-f.onrender.com/)
...
<img src="./assets/CUHKSZ_icon.png" alt="image-20251204230252549" style="zoom:50%;" />

![image-20251204225013102](./assets/dashborad_layout_1.png)

![image-20251204225104790](./assets/dashborad_layout_2.png)

![image-20251204225221648](./assets/dashborad_layout_3.png)
...
- **Top N 控制**：支持实时切换排名指标（浏览/加购/购买）与 Top N 数量（3-30）
  - 所有指标按固定顺序显示：浏览 → 加购 → 购买
- **日期范围筛选**：支持选择日期范围进行数据筛选
  - 预设快捷选项：数据集全部、2015年5-7月、2015年6-9月、最近三个月
...
- **All（全部用户）**：所有用户群体的汇总数据
- **Hesitant（犹豫型）**：浏览较多但转化率较低，需要更多引导。浏览多但购买少，购买率低于 5%
- **Impulsive（冲动型）**：浏览后快速加购和购买，转化率高。购买率高于 30%，24 小时内完成购买
- **Collector（收藏型）**：喜欢加购但购买决策周期较长。加购次数 ≥ 5 次，购买率 ≥ 10%

> 提示：在筛选栏中悬浮在用户群体卡片上可查看详细说明

## 安装和运行

### 方式一：在线访问

直接访问云端部署地址：[**https://cswun-datavis-dashboard-f.onrender.com/**](https://cswun-datavis-dashboard-f.onrender.com/)
...
│   │   │   ├── CUHKSZIcon.tsx              # CUHKSZ 自定义图标组件
│   │   │   ├── CUHKSZIcon.css              # CUHKSZ 图标样式和动画
│   │   │   ├── CUHKSZBackground.tsx        # CUHKSZ 背景组件
│   │   │   └── WeekdayDetailDrawer.tsx     # 星期几详情抽屉
│   │   ├── hooks/
│   │   │   ├── useResizableDrawer.tsx      # 可拖拽调整大小的抽屉 Hook
│   │   │   └── useResizeChart.ts            # 图表响应式调整 Hook
│   │   ├── App.tsx                 # 主应用组件
│   │   ├── App.css                 # 应用样式
│   │   └── main.tsx                # 应用入口
│   ├── package.json
...
- **Day.js**: 日期处理库
- **html2canvas**: 用于非 ECharts 组件的图片导出
...
7. **优化的图片导出**
   使用 ECharts 原生 canvas 导出功能，确保图表导出时图片质量高且无错位问题。
...
7. **抽屉功能**：所有抽屉（Drawer）支持从左侧边界拖拽调整宽度，范围 300-1200px
8. **布局保存**：图表布局会自动保存到浏览器本地存储，刷新后保持
